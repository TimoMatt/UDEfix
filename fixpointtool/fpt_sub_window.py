...
class FPTSubWindow(NodeEditorWidget):
    ...
    def doEvalOutputs(self):
        ...
        for node in self.scene.nodes:
            if node.__class__.__name__ == "FPTNode_Output":
                node.eval()  # TODO Maybe have to eval more nodes
    ...
    def initNewNodeActions(self):
        ...
        for key in keys:
            node = FPT_NODES[key]
            if not node.op_code == OP_CODE_SEPARATOR:
                self.node_actions[node.op_code] = QAction(QIcon(node.icon), node.op_title)
                self.node_actions[node.op_code].setData(node.op_code)
            else:
                self.node_actions[node.op_code] = QAction("------------------------")
                self.node_actions[node.op_code].setDisabled(True)
    ...
    def initNodesContextMenu(self):
        ...
        keys.sort()
        for key in keys:
            # if not FPT_NODES[key].op_code == OP_CODE_SEPARATOR:
    ...
    def handleNewNodeContextMenu(self, event):
        ...
        if action is not None:
            ...
            if self.scene.getView().mode == MODE_EDGE_DRAG:
                target_socket = self.determine_target_socket_of_node(self.scene.getView().dragging.drag_start_socket.is_output,
                                                                     new_calc_node)
